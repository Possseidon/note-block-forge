...
use crate::NoteBlockForge;

#[derive(Clone, Copy, Debug)]
pub(crate) struct ActionInfo {
    pub(crate) name: &'static str,
    pub(crate) executor: Option<ActionExecutor>,
    pub(crate) icon: Option<fn() -> ImageSource<'static>>,
...
struct ActionContextMut<'a> {
    state: &'a mut NoteBlockForge,
    context: &'a Context,
}

#[derive(Clone, Copy)]
struct ActionContext<'a> {
    state: &'a NoteBlockForge,
}

impl ActionInfo {
    pub(crate) fn all() -> impl Iterator<Item = Self> {
        (0..Action::LENGTH).map(|i| Action::from_usize(i).info())
    }
    ...
    const fn icon(self, icon: fn() -> ImageSource<'static>) -> Self {
        Self {
            icon: Some(icon),
            ..self
        }
...
impl Action {
    pub(crate) fn ui(self, ui: &mut Ui, state: &mut NoteBlockForge) -> Response {
        let info = self.info();

        let button =
            Button::opt_image_and_text(info.icon.map(|icon| icon().into()), Some(info.name.into()));
        if let Some(ActionExecutor {
            execute,
            can_execute,
        }) = info.executor
        {
            let response = ui.add_enabled(can_execute(ActionContext { state }), button);
            if response.clicked() {
    ...
    pub(crate) const fn info(self) -> ActionInfo {
        match self {
            Action::About => ActionInfo::new("About")
                ...
                .icon(|| include_image!("../assets/minecraft/block/redstone_torch.png"))
                .on_key(Key::F1)
                .execute(about),
            Action::CloseEditor => ActionInfo::new("Close Editor")
                .help("Close the current editor tab")
                .icon(|| include_image!("../assets/minecraft/block/oak_trapdoor.png"))
                .on_command(Key::N)
            ...
            Action::FixAudio => ActionInfo::new("Fix Audio")
                ...
                .icon(|| include_image!("../assets/minecraft/item/music_disc_11.png"))
                .on_key(Key::F7),
            ...
            Action::OpenIssue => ActionInfo::new("Open Issue")
                ...
                .icon(|| include_image!("../assets/minecraft/block/jack_o_lantern.png"))
                .execute(open_issue),
            Action::Play => ActionInfo::new("Play")
                .help("Start playback")
                .icon(|| include_image!("../assets/minecraft/item/music_disc_cat.png"))
                .on_key(Key::Space),
            Action::Save => ActionInfo::new("Save")
                .help("Save the current NoteBlockFile")
                .icon(|| include_image!("../assets/minecraft/item/writable_book.png"))
                .on_command(Key::S),
...
fn about(ctx: ActionContextMut) {
    ctx.state.windows.about_dialog ^= true;
}

fn midi_devices(ctx: ActionContextMut) {
    ctx.state.windows.midi_devices.main_dialog ^= true;
}
